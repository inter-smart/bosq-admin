...
} from "@/services/product/productAttributesApi";
...
export default function ProductAttributesForm() {
  ...
  useEffect(() => {
    if (isEditing && id) {
      loadAttributeData(parseInt(id));
    }
  }, [id, isEditing]);

  const loadAttributeData = async (itemId: number) => {
    try {
      setInitialLoading(true);
      const response = await fetchProductAttributeById(itemId);
      ...
      toast({
        title: "Error",
        description: "Failed to load attribute data",
        variant: "destructive",
      });
    } finally {
  ...
  const onSubmit = async (data: ProductAttributeFormData) => {
    try {
      setLoading(true);
      ...
      if (isEditing && id) {
        await updateProductAttribute(parseInt(id), payload);
        toast({
          title: "Success",
      ...
      toast({
        ...
        description:
          error.message ||
          `Failed to ${isEditing ? "update" : "create"} product attribute`,
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  if (initialLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-muted-foreground">Loading attribute data...</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button
          variant="outline"
          size="icon"
          onClick={() => navigate("/product-attributes")}
        >
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold">
            {isEditing ? "Edit" : "Add"} Product Attribute
          </h1>
          <p className="text-muted-foreground">
            {isEditing ? "Update" : "Create a new"} product attribute
          </p>
        </div>
      </div>
