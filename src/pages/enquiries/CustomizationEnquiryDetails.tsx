...
import { ArrowLeft, Trash2, Mail, Phone, User, MessageSquare, Calendar, Building2, Palette } from "lucide-react";
import {
  fetchCustomizationEnquiryById,
...
export default function CustomizationEnquiryDetails() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { toast } = useToast();
  const [enquiry, setEnquiry] = useState<CustomizationEnquiry | null>(null);
  ...
  const loadEnquiry = async (enquiryId: number) => {
    try {
      setLoading(true);
      const response = await fetchCustomizationEnquiryById(enquiryId);
      if (response.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="icon" onClick={() => navigate("/customization-enquiries")}>
      ...
      <div className="grid gap-6 md:grid-cols-2">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-base">{enquiry.first_name} {enquiry.last_name}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-base">{enquiry.company_name}</p>
              </div>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <p><span className="font-semibold">Region: </span>{enquiry.state?.name}</p>
            </div>
          </CardContent>
        </Card>
      </div>
