...
export default function ProductEnquiriesList() {
  ...
  const loadEnquiries = async () => {
    try {
      ...
      const response = await fetchProductEnquiries(
        currentPage,
        pageSize,
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteProductEnquiry(deleteItemId);
      toast({
        title: "Success",
  ...
  const handleExport = async (type: "csv" | "excel" | "pdf", selectedRows?: ProductEnquiry[]) => {
    ...
    try {
      ...
      if (!dataToExport || dataToExport.length === 0) {
        const response = await fetchProductEnquiries(1, 100000, debouncedSearchQuery, startDate, endDate);
        if (response.success) {
      ...
      exportToExcel(formattedData, `product_enquiries_${dateStr}`, 'Product Enquiries', columnWidths);
  ...
  const columns: ColumnDef<ProductEnquiry>[] = [
    {
      accessorKey: "id",
      header: "ID",
      cell: ({ row }) => (
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() => navigate(`/product-enquiries/${item.id}`)}
              >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              product enquiry and remove its data from the servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
