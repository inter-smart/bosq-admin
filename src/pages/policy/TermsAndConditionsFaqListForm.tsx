...
import { FaqListFormData, faqListSchema } from "@/schemas/termsAndConditionsSchema";
...
export default function FaqListForm() {
  ...
  const loadFaqData = async (itemId: number) => {
    try {
      ...
      toast({
        title: "Error",
        description: error?.message || "Failed to load FAQ data",
        variant: "destructive",
      });
      navigate("/terms-and-conditions-faq");
    } finally {
  ...
  const onSubmit = async (data: FaqListFormData) => {
    try {
      ...
      let response;
      if (isEditing && id) {
        response = await updateFaqList(parseInt(id), payload as any);
      } else {
        response = await createFaqList(payload as any);
      }

      if (response.success) {
        toast({
          title: "Success",
          description: response.message || `FAQ ${isEditing ? "updated" : "created"} successfully`,
        });
        navigate("/terms-and-conditions-faq");
      }
    } catch (error: any) {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button
          variant="outline"
          size="icon"
          onClick={() => navigate("/terms-and-conditions-faq")}
        >
