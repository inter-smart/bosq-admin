...
import {
  fetchFindYourFitById,
  createFindYourFit,
  updateFindYourFit,
} from "@/services/cms/home/findYourFitsApi";
import { findYourFitSchema, FindYourFitFormData } from "@/schemas/homeSchema";
import { Switch } from "@/components/ui/switch";

export default function FindYourFitsForm() {
  ...
  useEffect(() => {
    if (isEditing && id) {
      loadFitData(parseInt(id));
    }
  }, [id, isEditing]);

  const loadFitData = async (itemId: number) => {
    try {
      setInitialLoading(true);
      const response = await fetchFindYourFitById(itemId);
      ...
      toast({
        title: "Error",
        description: "Failed to load Find Your Fit data",
        variant: "destructive",
      });
    } finally {
  ...
  const onSubmit = async (data: FindYourFitFormData) => {
    try {
      ...
      if (isEditing && id) {
        await updateFindYourFit(parseInt(id), formData);
        toast({
          title: "Success",
          description: "Find Your Fit updated successfully",
        });
      } else {
        await createFindYourFit(formData);
        toast({
          title: "Success",
          description: "Find Your Fit created successfully",
        });
      }

      navigate("/find-your-fits");
    } catch (error) {
      toast({
        title: "Error",
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button
          variant="outline"
          size="icon"
          onClick={() => navigate("/find-your-fits")}
        >
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold">
            {isEditing ? "Edit" : "Add"} Find Your Fit
          </h1>
          <p className="text-muted-foreground">
            {isEditing ? "Update" : "Create a new"} Find Your Fit item
          </p>
        </div>
      </div>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <FileUpload
                          ...
                          accept="image/*"
                          preview={true}
                          recommendedDimensions="350px × 445px"
                        />
                      </FormControl>
                      <FormDescription>
                        {isEditing
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Add an optional link for this item
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
