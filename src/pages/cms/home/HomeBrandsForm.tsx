...
import { homeBrandSchema, HomeBrandFormData } from "@/schemas/homeSchema";
...
export default function HomeBrandsForm() {
  ...
  useEffect(() => {
    if (isEditing && id) {
      loadBrandData(parseInt(id));
    }
  }, [id, isEditing]);

  const loadBrandData = async (itemId: number) => {
    try {
      setInitialLoading(true);
      const response = await fetchHomeBrandById(itemId);
  ...
  const onSubmit = async (data: HomeBrandFormData) => {
    try {
      ...
      if (isEditing && id) {
        await updateHomeBrand(parseInt(id), formData);
        toast({
          title: "Success",
          description: "Brand updated successfully",
        });
      } else {
        await createHomeBrand(formData);
        toast({
          title: "Success",
      ...
      toast({
        title: "Error",
        description: `Failed to ${isEditing ? "update" : "create"} brand`,
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  if (initialLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-muted-foreground">Loading brand data...</div>
      </div>
    );
  }
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="Enter brand title (e.g., BMW, PORSCHE)"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <FileUpload
                        ...
                        accept="image/*"
                        preview={true}
                        recommendedDimensions="150px x 50px"
                      />
                    </FormControl>
                    <FormDescription>
                      {isEditing
                        ? "Upload a new logo to replace the current one (optional)"
                        : "Upload a brand logo image (required)"}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
