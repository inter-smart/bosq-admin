...
import { ProjectCategoryFormData, projectCategorySchema } from "@/schemas/projectsSchema";
...
export default function ProjectCategoryForm() {
  ...
  const loadCategoryData = async (itemId: number) => {
    try {
      ...
      const response = await fetchProjectCategoryById(itemId);
  ...
  const onSubmit = async (data: ProjectCategoryFormData) => {
    try {
      ...
      if (isEditing && id) {
        await updateProjectCategory(parseInt(id), payload);
        toast({
          title: "Success",
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">
            {isEditing ? "Edit" : "Add"} Project Category
          </h1>
          <p className="text-muted-foreground">
            {isEditing ? "Update" : "Create a new"} project category
          </p>
        </div>
      </div>
