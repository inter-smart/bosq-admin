...
import {
  fetchProjectImageById,
  createProjectImage,
  updateProjectImage,
} from "@/services/cms/projects/projectImagesApi";
import { Switch } from "@/components/ui/switch";
import {
  ProjectImagesFormData,
  projectImagesSchema,
} from "@/schemas/projectImagesSchema";
...
export default function ProjectImagesForm() {
  ...
  const form = useForm<ProjectImagesFormData>({
    resolver: zodResolver(projectImagesSchema),
  ...
  useEffect(() => {
    ...
    if (isEditing && id) {
      loadProjectImageData(parseInt(id));
    }
  }, [id, isEditing]);

  const loadProjectImageData = async (itemId: number) => {
    try {
      setInitialLoading(true);
      const response = await fetchProjectImageById(itemId);
  ...
  const onSubmit = async (data: ProjectImagesFormData) => {
    try {
      setLoading(true);
      ...
      if (isEditing && id) {
        await updateProjectImage(parseInt(id), formData);
        toast({
          title: "Success",
          description: "Project image updated successfully",
        });
      } else {
        await createProjectImage(formData);
        toast({
          title: "Success",
      ...
      navigate("/project-images?projectId=" + data.project_id);
    } catch (error: any) {
      toast({
        title: "Error",
        description:
          error.message ||
          `Failed to ${isEditing ? "update" : "create"} project image`,
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button
          variant="outline"
          size="icon"
          onClick={() => navigate(`/project-images?projectId=${parsedProjectId}`)}
        >
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold">
            {isEditing ? "Edit" : "Add"} Project Image
          </h1>
          <p className="text-muted-foreground">
            {isEditing ? "Update" : "Create a new"} project image
          </p>
        </div>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>
                          Enable or disable this project image
                        </FormDescription>
                      </div>
                      <FormControl>
                        <Switch
          ...
          <div className="flex justify-end gap-4">
            <Button type="button" variant="outline" onClick={() => navigate(`/project-images?projectId=${parsedProjectId}`)}>
              Cancel
            </Button>
