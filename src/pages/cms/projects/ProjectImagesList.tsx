...
import { fetchProjectImagesList, deleteProjectImage, ProjectImage } from "@/services/cms/projects/projectImagesApi";
...
export default function ProjectImagesList() {
  ...
  const { editingSortOrder, handleStatusChange, handleSortOrderChange } = useCommonTableActions<ProjectImage>({
  ...
  const loadProjectImages = async () => {
    try {
      ...
      const response = await fetchProjectImagesList(currentPage, pageSize, debouncedSearchQuery || undefined, projectId);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setProjectImages((prev) => prev.filter((item) => item.id !== deleteItemId));
      toast({
        title: "Success",
  ...
  const columns: ColumnDef<ProjectImage>[] = [
    ...
    },
    {
      ...
      header: "Alt Text (EN)",
      cell: ({ row }) => <div className="font-medium max-w-[200px] truncate">{row.getValue("media_alt") || "N/A"}</div>,
    },
    {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => navigate(`/project-images/edit/${item.id}?projectId=${projectId}`)}>
      ...
      <AlertDialog open={!!deleteItemId} onOpenChange={() => setDeleteItemId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This action cannot be undone. This will permanently delete the project image.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
