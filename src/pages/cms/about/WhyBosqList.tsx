...
export default function WhyBosqList() {
  ...
  const loadWhyBosqItems = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to load Why BOSQ items",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteWhyBosq(deleteItemId);
      setWhyBosqItems((prev) => prev.filter((item) => item.id !== deleteItemId));
      toast({
        title: "Success",
        description: "Why BOSQ item deleted successfully",
      });
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to delete Why BOSQ item",
        variant: "destructive",
      });
    } finally {
  ...
  const columns: ColumnDef<WhyBosq>[] = [
    {
      accessorKey: "id",
      header: "ID",
      cell: ({ row }) => (
          ...
          <div className="w-12 h-12 rounded-md flex items-center justify-center">
            {row.getValue("media_path") ? (
              <img
                ...
                alt={row.original.icon_media_alt || "media alt"}
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() => navigate(`/why-bosq/edit/${item.id}`)}
              >
  ...
  return (
    <>
      <DataTable
        ...
        addButtonText="Add Why BOSQ Item"
      />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              Why BOSQ item and remove its data from the servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
