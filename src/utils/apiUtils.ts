...
export const createItem = (endpoint: string) => (data: Record<string, any>) => apiCall(endpoint, { method: "POST", data });

export const updateItem = (endpoint: string) => (id: string | number, data: Record<string, any>) =>
  apiCall(`${endpoint}/${id}`, { method: "PUT", data });

export const deleteItem = (endpoint: string) => (id: string | number) => apiCall(`${endpoint}/${id}`, { method: "DELETE" });

export const fetchItem = (endpoint: string) => (id: string | number) => apiCall(`${endpoint}/${id}`);

export const fetchItems = (endpoint: string) => (params?: Record<string, string | number>) => apiCall(endpoint, { params });

// Bulk operations
export const bulkUpdate = (endpoint: string) => (data: { ids: number[]; updates: Record<string, any> }) =>
  apiCall(`${endpoint}/bulk`, { method: "PATCH", data });

export const bulkDelete = (endpoint: string) => (ids: number[]) => apiCall(`${endpoint}/bulk`, { method: "DELETE", data: { ids } });
